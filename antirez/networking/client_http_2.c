...
void initCtx(Ctx *ctx) {
  ...
  ctx->parser.p = ctx->parser.header_accumulator;
  ctx->parser.num_cr = 0;
...
int checkCapacity(Ctx *ctx) {
  /*
  faccio un for loop per vedere se in recv c'è \n.
  ...
  */
  ...
  impostare lo stato del parser. Se non trovo il byte successivo leggo ancora da
  recv altrimenti posso iniziare il parsing.
  */
  ...
  while ((ctx->conn.numbytes = recv(ctx->conn.sockfd, ctx->conn.recv_buf,
    ...
    switch (ctx->parser.state) {
      case HEADER_STATUS_LINE: {
        int res = checkCapacity(ctx);
        // int res = parseStatusLine(ctx);
        if (res != 0) {
          continue;
        }
        parseStatusLine(ctx);
        ...
        // case HEADER_ACC: {
        //   if (ctx->parser.header_idx + numbytes > MAXHEADER) {
        //     fprintf(stderr, "Error: header too big");
        //     return 1;
        //   }
        //   memcpy(ctx->parser.header_accumulator + ctx->parser.header_idx,
        //          ctx->conn.recv_buf, numbytes);
        // }
        // cerco \r e verifico che ci sia almeno un byte successivo
        // se l'indice in cui trovo \r è uguale a header_idx devo leggere ancora
        // con recv(). Se è inferiore verifico che il byte successivo sia \n e
        // posso parsare HEADER_STATUS_LINE se num_cr = 0. Se num_cr > 0 devo
        // verificare se mi trovo alla fine dell'header oppure devo iniziare a
        // leggere una HEADER_KEY
    }
  }
